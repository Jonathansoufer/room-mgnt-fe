  ...
  return (
    <S.Container initial="initial" animate='animate' variants={fade}>
      <div style={{ display: 'flex', flex: 1, justifyContent: "space-between" }}>
        <div>
          <S.Title>Hello, <span>{profile?.user.name}</span>!</S.Title>
          <Link to="/appointments">
            <h2 style={{ color: '#000', textDecoration: 'underline' }} >Appointments</h2>
          </Link>
        </div>
        ...
        {rooms && rooms.map(room => (
          <>
            {room.company === profile?.user.company && (
              <Link style={{ display: 'flex', margin: 20 }} to={`/room/${room.id}`} >
